...
export function Demo4({ children }: { children: ReactNode }) {
      ...
      if (action === "run") {
        const stream = new ReadableStream({
          async start(controller) {
            ...
            controller.enqueue('0:{"promise":"$@1"}\n');

            const value = await promise;
            controller.enqueue(`1:"${value}"\n`);
            controller.enqueue("\n");
            controller.close();
          },
        });
  ...
  return (
    <div className="not-prose relative">
      ...
      <div
        className={clsx(
          "relative flex min-h-[158px] rounded-b-md border-x-2 border-b-2 border-gray-200",
