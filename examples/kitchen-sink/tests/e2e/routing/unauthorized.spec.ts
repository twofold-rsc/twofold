...
test("renders unauthorized after an async page resolves", async ({ page }) => {
  await page.goto("/routing/unauthorized/async-unauthorized");
...
test("navigates to unauthorized pages from the menu", async ({ page }) => {
  ...
  await expect(page).toHaveURL("/routing/unauthorized/page-calls-unauthorized");
  await expect(
    page.getByRole("heading", { name: "Not authorized", exact: true }),
  ).toBeVisible();
});
...
test("shows unauthorized after a server action", async ({ page }) => {
...
test.skip("todo: observe the loading fallback before suspended unauthorized resolves", async () => {});

test.skip("todo: assert the HTTP status when unauthorized occurs after streamed headers are committed", async () => {});
